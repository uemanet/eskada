 ...
 * @param null $chapter chapter object
            ...
            case 'readpercent':
                if (!empty($val)) {
                    if (!$book = $DB->get_record('book', ['id' => $cm->instance])) {
                        throw new \moodle_exception('Unable to find book with id ' . $cm->instance);
                    }

                    $descriptions[] = get_string('readpercent_ruledesc', 'mod_book', $book->readpercent);
                }
                break;
            default:
                break;
